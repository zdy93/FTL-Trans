...
def vote_score(df, score, id_name, mode, args):
    df['pred_score'] = score
    df['prediction'] = np.asarray([1 if i else 0 for i in (np.array(score) >= 0.5)])
    df_sort = df.sort_values(by=[id_name])
    # score
    if mode == "clinicalBERT":
        temp = (df_sort.groupby([id_name])['pred_score'].agg(max) + df_sort.groupby([id_name])['pred_score'].agg(
            sum) / 2) / (1 + df_sort.groupby([id_name])['pred_score'].agg(len) / 2)
    elif mode == "Majority Vote":
        temp = df_sort.groupby([id_name])['prediction'].agg(lambda x: pd.Series.mode(x)[0])
    x = df_sort.groupby([id_name])['Label'].agg(np.min).values
    ids = df_sort[id_name].unique()
    df_out = pd.DataFrame({'logits': temp.values, 'Label': x, id_name: ids})
    ...
    plt.legend(loc='best')
    plt.show()
    string = 'auroc_clinicalbert_' + id_name + '.png'
...
def pr_curve_plot(y, y_score, id_name, args):
    ...
    string = 'auprc_clinicalbert_' + id_name + '.png'

    plt.savefig(os.path.join(args.output_dir, string))
    plt.close()


def vote_pr_curve(df, score, id_name, args):
    df['pred_score'] = score
    df_sort = df.sort_values(by=[id_name])
    # score
    temp = (df_sort.groupby([id_name])['pred_score'].agg(max) + df_sort.groupby([id_name])['pred_score'].agg(
        sum) / 2) / (1 + df_sort.groupby([id_name])['pred_score'].agg(len) / 2)
    y = df_sort.groupby([id_name])['Label'].agg(np.min).values

    precision, recall, thres = precision_recall_curve(y, temp)
    pr_thres = pd.DataFrame(data=list(zip(precision, recall, thres)), columns=['prec', 'recall', 'thres'])
    vote_df = pd.DataFrame(data=list(zip(temp, y)), columns=['score', 'label'])

    pr_curve_plot(y, temp, id_name, args)
...
def main():
    ...
    for step in range(te_ids_num):
        ...
        # logits = torch.squeeze(m(logits)).detach().cpu().numpy()[:, 1]
    ...
    #     matthews = matthews_corrcoef(true_labels[i],
    #                                  np.asarray([1 if j else 0 for j in (predictions[i].flatten() >= 0.5)]))
    ...
    # test_df['logits'] = flat_logits
    # note_score = test_df.groupby('Note_ID')['logits'].mean()
    # patient_score = test_df.groupby('Adm_ID')['logits'].mean()
    # note_prediction = (note_score >= 0.5).astype(int)
    # patient_prediction = (patient_score >= 0.5).astype(int)
    # note_label = test_df.groupby('Note_ID')['Label'].agg(pd.Series.mode)
    # patient_label = test_df.groupby('Adm_ID')['Label'].agg(pd.Series.mode)
    #
    # test_accuracy = accuracy_score(flat_true_labels, flat_predictions)
    # note_accuracy = accuracy_score(note_label, note_prediction)
    # patient_accuracy = accuracy_score(patient_label, patient_prediction)
    # write_log('''Simple Mean Result:
    #     Test Chunk Level Accuracy: {},
    #     Note Level Accuracy: {},
    #     Patient Level Accuracy: {}'''.format(test_accuracy, note_accuracy, patient_accuracy), LOG_PATH)
    #
    # test_mc = matthews_corrcoef(flat_true_labels, flat_predictions)
    # note_mc = matthews_corrcoef(note_label, note_prediction)
    # patient_mc = matthews_corrcoef(patient_label, patient_prediction)
    # write_log('''Simple Mean Result:
    #     Test Chunk Level Matthew's correlation coefficient: {},
    #     Note Level Matthew's correlation coefficient: {},
    #     Patient Level Matthew's correlation coefficient: {}'''.format(test_mc, note_mc, patient_mc), LOG_PATH)
    #
    # write_performance(patient_label, patient_prediction, patient_score, config, args)
