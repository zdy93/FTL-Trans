...
class T3LSTM(nn.Module):
    def __init__(self, input_size, hidden_size, config, batch_first=True, bidirectional=True):
        ...
        self.b_decomp = Parameter(torch.zeros(self.hidden_size))
        # Decay Parameter
        self.W_decay_1 = Parameter(torch.normal(0.0, config.initializer_range, size=(1, 1)))
        self.W_decay_2 = Parameter(torch.normal(0.0, config.initializer_range, size=(1, 1)))
        self.W_decay_3 = Parameter(torch.normal(0.0, config.initializer_range, size=(1, 1)))
